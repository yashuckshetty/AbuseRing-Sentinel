...
Unset GEMINI_API_KEY before recording; PolicyGate falls back to the deterministic template reasoner, keeping the audit output byte-stable across takes.
...
1. **System Health & Model Ladder (Top Header)**
   ...
   - Inspect **Model Ladder (Rungs 1-5, plus Rung 6 GNN comparison)**:
     - Rung 3 (Behavioral LGBM): F1 Macro 0.8959, Test AC Recall 95.45% ([`evals/metrics.json`](evals/metrics.json)).
     - Rung 4 (Structural LGBM): F1 Macro 0.7647, Test AC Recall 23.74% ([`evals/metrics.json`](evals/metrics.json)).
     - Rung 5 (Calibrated Fused): 0 False Positives in auto-ACT lane, 81.82% effective recall via routing (162/198 AC accounts: 38 auto-ACT + 124 REVIEW; [`evals/results/adversarial_results.json`](evals/results/adversarial_results.json)).
